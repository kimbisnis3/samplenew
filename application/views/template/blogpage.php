...
<article class='post-entry post-entry-type-standard post-entry-3772 post-loop-1 post-parity-odd single-big with-slider post-3772 post type-post status-publish format-standard has-post-thumbnail hentry category-blog category-frontpage-article category-news'  itemscope="itemscope" itemtype="https://schema.org/BlogPosting" itemprop="blogPost" ><div class="big-preview single-big"><a href="<?php echo base_url('/'); ?><?php echo $menu ?>/<?php echo $t->slug ?>" title="<?php echo $t->judul ?>"><img width="845" height="321" src="<?php echo base_url('gopanel'); ?><?php echo $t->gambar ?>" class="attachment-entry_with_sidebar size-entry_with_sidebar wp-post-image" alt="<?php echo $t->judul ?>" /></a></div><div class='blog-meta'></div><div class='entry-content-wrapper clearfix standard-content'>

<header class="entry-content-header"><h2 class='post-title entry-title'  itemprop="headline" >	<a href='<?php echo base_url('/'); ?><?php echo $menu ?>/<?php echo $t->slug ?>' rel='bookmark' title='Permanent Link:<?php echo $t->judul ?>'><!--judul--><?php echo $t->judul ?>			<span class='post-format-icon minor-meta'></span>	</a></h2>
...
<?php } ?>
    ...
    ?>
        ...
        <?php foreach ($recentpost as $t ) { ?>
            <li>
        <a href="<?php echo base_url('/'); ?><?php echo $menu ?>/<?php echo $t->slug ?>"><?php echo $t->judul; ?></a>
                </li>
        <?php } ?>
        </ul>
